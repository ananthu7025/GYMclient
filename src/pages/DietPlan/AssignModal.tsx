...
interface MemberSearchModalProps {
  ...
  DietPlanId: string; // Add workoutPlanId prop
}
...
export const MemberSearchModal: React.FC<MemberSearchModalProps> = ({
  ...
  const handleAssignDietPlan = async () => {
    ...
    try {
        const response = await axiosClient.post(API_URLS.ASSIGN_DIET_PLAN, {
            memberId: selectedMember.value,
            dietPlanId: DietPlanId, // Use the passed workoutPlanId
          });
          handleClose();
      ...
      setError(
        err.response?.data?.message ||
          "An error occurred while assigning the diet plan."
      );
    } finally {
      setLoading(false);
    }
      ...
      <Modal.Body>
        ...
        {!selectedMember && (
          <div className="text-center mb-4">
            ...
            />
            <h5 className="mt-2">Assign a diet plan to a Member </h5>
